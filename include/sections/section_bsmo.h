...
/**
 * Contains the immutable data to represent a node, including a parent relative
 * offset and initial transform.
 */
struct BSMONodeItem {
...
class SectionBSMO : public SectionBase {
 public:
  ...
  /**
   * Contains a relative index to a node that affects this render set, so we can
  ...
  /**
   * Contains an index to the nodes that the animation effects so they can be
   * stored sequentially.
   */
